...
const handler = async (req, res) => {
  ...
  try {
    if (req.method === "PATCH" && url.pathname === "/api/v1/account/oauth-profile") { const user = identity(req); const input = await body(req); const { data: current, error: currentError } = await supabase.from("users").select("oauth_provider").eq("id", user.userId).is("deleted_at", null).maybeSingle(); if (currentError) throw currentError; if (!current?.oauth_provider) return send(res, 403, { error: "OAUTH_PROFILE_REQUIRED" }); const allowed = ["full_name", "phone", "birth_date", "postal_code", "address", "address_detail", "twitter_handle"]; const updates = Object.fromEntries(allowed.filter((key) => input[key] !== undefined).map((key) => [key, input[key] || null])); if (!Object.keys(updates).length) return send(res, 400, { error: "NO_PROFILE_CHANGES" }); const { data, error } = await supabase.from("users").update(updates).eq("id", user.userId).select("id,email,role,twitter_handle,full_name,phone,birth_date,postal_code,address,address_detail").single(); if (error) return send(res, 400, { error: "PROFILE_UPDATE_FAILED" }); return send(res, 200, { user: data }); }
    ...
    if (req.method === "PATCH" && url.pathname.match(/^\/api\/v1\/customer\/orders\/[^/]+\/address-request$/)) { const customer = role(req, res, ["CUSTOMER"]); if (!customer) return; const orderId = url.pathname.split("/")[5]; const input = await body(req); const { data: order, error: orderError } = await supabase.from("orders").select("id,customer_id,status,shipping_info").eq("id", orderId).eq("customer_id", customer.userId).maybeSingle(); if (orderError) throw orderError; if (!order) return send(res, 404, { error: "ORDER_NOT_FOUND" }); if (["SHIPPED", "SETTLED", "RECEIVED"].includes(order.status)) return send(res, 409, { error: "ADDRESS_CHANGE_LOCKED" }); if (!input.postal_code || !input.address) return send(res, 400, { error: "ADDRESS_REQUIRED" }); const shipping = { ...(order.shipping_info || {}), new_zonecode: input.postal_code, new_address: input.address, new_detail_address: input.address_detail || "", address_change_status: "요청중", address_requested_at: new Date().toISOString() }; const { data, error } = await supabase.from("orders").update({ shipping_info: shipping }).eq("id", orderId).select("id,shipping_info").single(); if (error) throw error; return send(res, 200, { order: data }); }
    if (req.method === "PATCH" && url.pathname.match(/^\/api\/v1\/seller\/orders\/[^/]+\/address-approve$/)) { const seller = role(req, res, ["SELLER", "ADMIN"]); if (!seller) return; const orderId = url.pathname.split("/")[5]; const input = await body(req); const { data: order, error: orderError } = await supabase.from("orders").select("id,project_id,shipping_info").eq("id", orderId).maybeSingle(); if (orderError) throw orderError; if (!order) return send(res, 404, { error: "ORDER_NOT_FOUND" }); const project = await getOne("projects", order.project_id); if (!project || (project.leader_id !== seller.userId && seller.role !== "ADMIN")) return send(res, 403, { error: "PROJECT_ACCESS_FORBIDDEN" }); const shipping = { ...(order.shipping_info || {}) }; if (input.approve) { shipping.postal_code = shipping.new_zonecode; shipping.address = shipping.new_address; shipping.address_detail = shipping.new_detail_address; shipping.address_change_status = "승인"; } else { shipping.address_change_status = "거절"; shipping.address_reject_reason = input.reason || "총대가 변경 요청을 승인하지 않았습니다."; } delete shipping.new_zonecode; delete shipping.new_address; delete shipping.new_detail_address; const { data, error } = await supabase.from("orders").update({ shipping_info: shipping }).eq("id", orderId).select("id,shipping_info").single(); if (error) throw error; return send(res, 200, { order: data }); }
    if (req.method === "POST" && url.pathname.match(/^\/api\/v1\/seller\/orders\/[^/]+\/approve-deposit$/)) { const seller = role(req, res, ["SELLER", "ADMIN"]); if (!seller) return; const orderId = url.pathname.split("/")[5]; const { data: order, error: orderError } = await supabase.from("orders").select("id,project_id,status").eq("id", orderId).maybeSingle(); if (orderError) throw orderError; if (!order) return send(res, 404, { error: "ORDER_NOT_FOUND" }); const project = await getOne("projects", order.project_id); if (!project || (project.leader_id !== seller.userId && seller.role !== "ADMIN")) return send(res, 403, { error: "PROJECT_ACCESS_FORBIDDEN" }); const { error: paymentError } = await supabase.from("payments").update({ status: "HELD", paid_at: new Date().toISOString() }).eq("order_id", orderId).eq("status", "PENDING"); if (paymentError) throw paymentError; const { data, error } = await supabase.from("orders").update({ status: "PAYMENT_CONFIRMED" }).eq("id", orderId).select().single(); if (error) throw error; return send(res, 200, { order: data }); }
    if (req.method === "GET" && url.pathname === "/api/v1/customer/purchase-history") { const customer = role(req, res, ["CUSTOMER"]); if (!customer) return; const { data, error } = await supabase.from("orders").select("*, projects(id,title,status,updated_at), order_items(*), payments(*), reviews(id,rating)").eq("customer_id", customer.userId).order("created_at", { ascending: false }); if (error) throw error; return send(res, 200, { items: (data || []).map((order) => ({ ...order, project: order.projects, projects: undefined })) }); }
    if (req.method === "GET" && url.pathname === "/api/v1/account/addresses") { const user = identity(req); const { data, error } = await supabase.from("user_addresses").select("id,label,recipient_name,phone,postal_code,address,address_detail,is_default,created_at,updated_at").eq("user_id", user.userId).order("is_default", { ascending: false }).order("created_at", { ascending: false }); if (error) throw error; return send(res, 200, { items: data || [] }); }
    if (req.method === "POST" && url.pathname === "/api/v1/account/addresses") { const user = identity(req); const input = await body(req); const required = ["label", "recipient_name", "phone", "postal_code", "address"]; if (required.some((key) => !String(input[key] || "").trim())) return send(res, 400, { error: "ADDRESS_FIELDS_REQUIRED" }); const makeDefault = Boolean(input.is_default); if (makeDefault) { const { error: clearError } = await supabase.from("user_addresses").update({ is_default: false, updated_at: new Date().toISOString() }).eq("user_id", user.userId).eq("is_default", true); if (clearError) throw clearError; } const { data, error } = await supabase.from("user_addresses").insert({ user_id: user.userId, label: String(input.label).trim(), recipient_name: String(input.recipient_name).trim(), phone: String(input.phone).trim(), postal_code: String(input.postal_code).trim(), address: String(input.address).trim(), address_detail: String(input.address_detail || "").trim() || null, is_default: makeDefault }).select("id,label,recipient_name,phone,postal_code,address,address_detail,is_default,created_at,updated_at").single(); if (error) throw error; return send(res, 201, { address: data }); }
    if (req.method === "PATCH" && url.pathname.match(/^\/api\/v1\/account\/addresses\/[^/]+\/default$/)) { const user = identity(req); const addressId = url.pathname.split("/")[5]; const { data: target, error: targetError } = await supabase.from("user_addresses").select("id").eq("id", addressId).eq("user_id", user.userId).maybeSingle(); if (targetError) throw targetError; if (!target) return send(res, 404, { error: "ADDRESS_NOT_FOUND" }); const { error: clearError } = await supabase.from("user_addresses").update({ is_default: false, updated_at: new Date().toISOString() }).eq("user_id", user.userId); if (clearError) throw clearError; const { data, error } = await supabase.from("user_addresses").update({ is_default: true, updated_at: new Date().toISOString() }).eq("id", addressId).eq("user_id", user.userId).select("id,label,recipient_name,phone,postal_code,address,address_detail,is_default,created_at,updated_at").single(); if (error) throw error; return send(res, 200, { address: data }); }
    if (req.method === "DELETE" && url.pathname.match(/^\/api\/v1\/account\/addresses\/[^/]+$/)) { const user = identity(req); const addressId = url.pathname.split("/").pop(); const { data: target, error: targetError } = await supabase.from("user_addresses").select("id,is_default").eq("id", addressId).eq("user_id", user.userId).maybeSingle(); if (targetError) throw targetError; if (!target) return send(res, 404, { error: "ADDRESS_NOT_FOUND" }); if (target.is_default) return send(res, 409, { error: "DEFAULT_ADDRESS_CANNOT_DELETE" }); const { error } = await supabase.from("user_addresses").delete().eq("id", addressId).eq("user_id", user.userId); if (error) throw error; return send(res, 204, {}); }
