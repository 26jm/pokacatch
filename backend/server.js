...
async function parseWithUpstage(url, text) {
  ...
  const response = await fetch("https://api.upstage.ai/v1/solar/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      temperature: 0,
      messages: [
        { role: "system", content: "Extract group-buying information from the supplied X post. Return only valid JSON with keys group_name, goods_type, store_name, title, price, members, shipping_policy. Use null or [] when unknown." },
...
const handler = async (req, res) => {
  ...
  try {
    ...
    if (req.method === "GET" && url.pathname === "/api/v1/customer/payment-history") { const customer = role(req, res, ["CUSTOMER"]); if (!customer) return; const { data, error } = await supabase.from("payments").select("*").eq("user_id", customer.userId).order("created_at", { ascending: false }); if (error) throw error; return send(res, 200, { items: data || [] }); }
