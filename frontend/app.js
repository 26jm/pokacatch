...
const languageMessages = { ko: "언어가 변경되었습니다.", en: "Language changed.", ar: "تم تفعيل اللغة العربية.", ja: "言語を変更しました。", zh: "语言已切换。" };
...
byId("language-select").addEventListener("change", (event) => { state.language = event.target.value; setDocumentLanguage(state.language); toast(languageMessages[state.language] || languageMessages.ko); });
